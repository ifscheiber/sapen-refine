...
| Drag empty image space to draw a rough rectangle around a slice. | A slice proposal appears in the BBox status rail and is clearly labeled as a proposal, not ground truth. |  |  |
...
| Click `Semantic Masks`. | Slice preparation runs automatically and the unified crop annotation editor opens directly with Sapwood/Heartwood and Cu family choices, support/semantic/classification/readiness status, and the whole-image slice navigator. |  |  |
...
| From the crop editor, click `Edit BBoxes`. | Browser lands on `/crop/bboxes`; confirmed BBoxes are visible, editable BBoxes can be moved directly, and no `Editor` or `Full editor` crop-workflow link is visible. |  |  |
| Replace or delete an editable BBox, then leave the BBox tab for `Semantic Masks`. | The workflow changes to `BBOX_NEEDS_UPDATE`, the tab switch prepares slices automatically, and the browser returns to crop annotation after preparation. |  |  |
| Try to move or delete a BBox that already has semantic/support/classification data. | A confirmation dialog explains that downstream annotations will be invalidated; cancel preserves data, confirm supersedes downstream versions and permits the BBox change. |  |  |
...
| If testing the RB-112 worker path, run one script pass for due exports. | `SAPEN_JOB_EMAIL='<owner-or-qa-email>' SAPEN_JOB_PASSWORD_FILE=/run/secrets/sapen_job_password npm run exports:process -- --max-jobs 2` processes due exports without a browser tab and logs processor/run summary without secrets. |  |  |
...
| If testing the RB-065 worker path, run one script pass for due batches. | `SAPEN_JOB_EMAIL='<owner-or-qa-email>' SAPEN_JOB_PASSWORD_FILE=/run/secrets/sapen_job_password npm run jobs:prediction-import -- --limit 25 --max-jobs 5` processes due batches without a browser tab and logs processor/run summary without secrets. |  |  |
| If testing RB-066 cleanup, run storage cleanup dry-run after batch processing. | `SAPEN_CLEANUP_EMAIL='<admin-email>' SAPEN_CLEANUP_PASSWORD_FILE=/run/secrets/sapen_cleanup_password npm run storage:cleanup -- --category batch-staging --batch '<batch-id>'` reports candidates without deleting committed raw images, prediction artifacts, or exports. |  |  |
...
- RB-086/RB-087/RB-088/RB-089/RB-090/RB-091/RB-092/RB-096/RB-100/RB-103/RB-123 support source-image BBox slice proposals, derived crop previews, direct unified crop editing, crop support-mask editing, mode-aware crop semantic editing, byte-derived annotation-family guards, draft auto classification suggestions/manual overrides, crop training export, crop-aware review/readiness integration, and explicit BBox re-entry after crop inspection.
