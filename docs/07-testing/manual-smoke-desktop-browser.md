...
| Save a Sap/Heartwood semantic draft, then open Copper for the same crop. | Copper opens read-only with a semantic-family reset warning; cancelling reset preserves the Sap/Heartwood family. Confirmed reset arms the next Copper save and keeps old versions historical. |  |  |
