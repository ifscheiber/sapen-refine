...
| Attempt to switch the same crop from Sapwood/Heartwood to Cu after saving Sapwood/Heartwood pixels. | Cu remains unavailable until Sapwood/Heartwood pixels are cleared and the crop cannot silently become mixed-family ready. |  |  |
...
| Open project exports and create a training export as owner. | `/app/projects/[projectId]/exports` readiness counts include the approved components, export creation shows a pending/processing state, and manifest/package download links appear after the export worker completes the job. Integrity warnings block export creation if selected inputs lack checksum/dimensions. |  |  |
...
- RB-053/RB-091/RB-112 training exports and RB-060/RB-067/RB-112 prediction-analysis exports use async single-host jobs and remain trial-sized behind caps. RB-061/RB-065 prediction batch imports use bounded explicit or optional worker process passes. RB-066 storage cleanup is admin-only and dry-run first. Cleanup UI, production-scale queue infrastructure, advanced export filters/history, metrics dashboards, and streaming export packaging remain deferred.
