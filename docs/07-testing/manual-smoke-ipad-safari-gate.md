...
| Attempt to switch that crop to Copper. | A semantic-family reset confirmation appears; cancelling reset keeps Sap/Heartwood active and controls remain usable. | Yes if crop workflow is part of pilot |  |  |
| On a fresh crop or after explicit reset, draw and save a Copper crop semantic draft before support exists. | Save succeeds, auto classification is Copper slice, and readiness remains blocked by missing approved explicit support. | Yes if crop workflow is part of pilot |  |  |
