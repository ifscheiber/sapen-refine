...
| Click `Prepare slices`, then `Open slice annotation`. | The unified crop annotation editor opens directly with Sapwood/Heartwood and Cu family controls, status badges, readiness guidance, and whole-image slice navigation without layout overlap. | Yes if crop workflow is part of pilot |  |  |
