...
| Click `Prepare slices`, then `Open slice annotation`. | The unified crop annotation editor opens directly with Sapwood/Heartwood and Cu/Support family controls, status badges, readiness guidance, and whole-image slice navigation without layout overlap. | Yes if crop workflow is part of pilot |  |  |
| Tap `Cu / Support mask` and `Support` for the generated crop. | The editor displays the crop image, keeps drawing in crop coordinates, and uses support/background labels. | Yes if crop workflow is part of pilot |  |  |
...
| Attempt to switch that crop to Copper. | Cu/Support remains unavailable until Sapwood/Heartwood pixels are cleared and controls remain usable. | Yes if crop workflow is part of pilot |  |  |
