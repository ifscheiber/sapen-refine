...
| Tap `Semantic Masks`. | Slice preparation runs automatically and the unified crop annotation editor opens directly with Sapwood/Heartwood and Cu family controls, status badges, readiness guidance, and whole-image slice navigation without layout overlap. | Yes if crop workflow is part of pilot |  |  |
| Tap `Cu` and `Support` for the generated crop. | The editor displays the crop image, keeps drawing in crop coordinates, uses support/background labels, and limits support drawing tools to Polygon/Lasso. | Yes if crop workflow is part of pilot |  |  |
| Draw and erase a crop support mask, then save. | A draft crop support mask saves and reloads; only support/background labels are available and review actions fit the viewport. | Yes if crop workflow is part of pilot |  |  |
| Tap a semantic target in the generated crop editor. | The editor shows the support overlay where applicable and mode/tool controls fit the viewport. | Yes if crop workflow is part of pilot |  |  |
| Test the supportless Sap/Heartwood path on a crop without relying on explicit support. | Save succeeds, reload preserves the draft, support geometry derives from semantic foreground, and a Sap/Heartwood auto classification suggestion appears. | Yes if crop workflow is part of pilot |  |  |
| Attempt to switch that crop to Copper. | Cu remains unavailable until Sapwood/Heartwood pixels are cleared and controls remain usable. | Yes if crop workflow is part of pilot |  |  |
| On a fresh crop or after clearing Sapwood/Heartwood, draw and save a Copper crop semantic draft before support exists. | Save succeeds, auto classification is Copper slice, and readiness remains blocked by missing approved explicit support. | Yes if crop workflow is part of pilot |  |  |
| Draw and approve explicit support for a Copper crop, then approve Copper semantic and classification. | Copper crop readiness can become ready only after approved support, approved semantic, and approved classification exist. | Yes if crop workflow is part of pilot |  |  |
