...
- `SAPEN_OPERATOR_EMAIL`, `SAPEN_REQUIRE_OPERATOR_ATTRIBUTION`, `SAPEN_ALLOW_LOCAL_SYSTEM_ACTOR` - optional trial bootstrap/script attribution controls consumed by `scripts/trial-bootstrap.mjs`, `scripts/create-trial-user.mjs`, and `scripts/deactivate-trial-user.mjs`; customer-trial/prod commands must use a named operator identity, and the local system actor fallback must be explicitly enabled.
- `SAPEN_DATASET_BASE_URL`, `SAPEN_DATASET_EMAIL`, `SAPEN_DATASET_PASSWORD_FILE`, `SAPEN_DATASET_PASSWORD` - optional SaPen-CNN materializer fetch credentials for `scripts/materialize-sapen-cnn-dataset.mjs`; prefer `SAPEN_DATASET_PASSWORD_FILE`.
...
- `SAPEN_TRIAL_USER_PASSWORD_FILE`, `SAPEN_TRIAL_USER_PASSWORD` - optional trial-user creation password input when running `npm run trial:user:create` through Compose. Prefer the file variable.
- `SAPEN_OPERATOR_EMAIL`, optionally `SAPEN_REQUIRE_OPERATOR_ATTRIBUTION=true` - named operator attribution for trial bootstrap, trial-user creation, and user deactivation. Do not put passwords, tokens, or file-secret paths in this value.
...
- Export job attempts: `3`.
- Export job retry delay: `60` seconds.
- Export job lease timeout: `900` seconds.
- Export due jobs per worker tick: `2`.
- Export worker loop interval: `30` seconds.
