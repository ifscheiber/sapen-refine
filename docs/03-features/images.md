...
- RB-089/RB-100 creates crop-scoped semantic mask versions linked to the source image, `DerivedSliceCrop`, `SliceInstance`, optional support mask version, and semantic mode. Sap/Heartwood can be supportless and derives support geometry from semantic foreground; Copper needs explicit support for readiness/export.
- RB-090 derives draft slice-classification suggestions from saved crop semantic masks and stores source/reason plus links back to the semantic mask, optional support mask, and crop. Manual overrides append separate `MANUAL` classification versions for the slice instance.
...
- `POST /api/slice-crops/[cropId]/semantic-mask/upload` uploads crop-sized semantic bytes through the app server, records a crop-linked `SEMANTIC_MASK` artifact version for `x-semantic-mode` with optional `x-support-mask-version-id`, and appends a draft auto classification suggestion.
