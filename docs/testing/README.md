...
- `tests/unit/upload-validation.test.ts` covers image/mask upload size validation, `413` payloads, SHA-256 checksum normalization, PNG/JPEG dimension parsing, image content-type rejection, mask dimension checks, support-mask value validation, and RB-080 diagnostic-only declared mask byte length handling.
...
- `tests/e2e/large-mask-upload.spec.ts` covers RB-080 full-resolution editor upload behavior with a synthetic valid 6000x4000 PNG. It waits for editor readiness, draws a tiny semantic stroke, intercepts the app-mediated save request, and asserts `x-mask-width`, `x-mask-height`, `x-mask-format`, diagnostic `x-mask-byte-length`, and a 24,000,000-byte request body.
