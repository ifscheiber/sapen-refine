...
- `tests/unit/slice-domain.test.ts` covers RB-051 support artifact kind helpers, latest classification resolution, and RB-086 BBox validation errors.
...
- `tests/unit/editor-helpers.test.ts` covers RB-068 extracted editor API path builders, review/classification/correction display helpers, abort detection, pointer ignore decisions, RB-070 eraser tool/value behavior, and RB-080 exact editor mask upload payload construction for large masks and typed-array views.
...
- `tests/unit/mask-upload-route-contracts.test.ts` covers RB-081 route-level use of the shared raw mask upload reader across semantic, support, and assisted-correction save paths.
...
- `tests/e2e/slice-bbox-proposals.spec.ts` covers RB-086 editor BBox proposal creation, persistence through `/api/images/[imageId]/slice-bboxes`, and reload visibility.
...
- RB-086 adds the first crop-sprint runtime coverage for source-image BBox proposal validation, persistence, and editor reload. Crop generation, crop mask dimensions, crop-aware exports, and crop review eligibility remain deferred to RB-087 through RB-092.
