...
- `tests/integration/prediction-import-batches.test.ts` covers RB-061 ZIP batch creation, private item staging, partial success/failure summaries, stable item error codes, retry reset behavior, idempotent reprocessing, role authorization, staging-key response sanitization, unsupported target rejection, and export exclusion for imported predictions.
