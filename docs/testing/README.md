...
- `tests/e2e/api-error-contracts.spec.ts` covers RB-072 browser/API-level JSON errors for unauthenticated project access plus authenticated forbidden/not-found cases across export, image metadata, prediction run, and storage cleanup routes. It also covers RB-105 disabled legacy presign/commit upload route errors and RB-079 stale workspace session redirect behavior.
