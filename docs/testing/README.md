...
- `tests/e2e/crop-workflow-closeout.spec.ts` covers the RB-098/RB-123/DESIGN-014 crop workflow closeout gaps: Copper semantic drafts can be saved before explicit support but remain non-ready until support/semantic/classification review is complete, redundant Classification tab/manual crop classification controls stay hidden, and active Sapwood/Heartwood annotation disables the Cu/Support family.
