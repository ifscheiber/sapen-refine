...
This page records the current repository state. It incorporates the RB-049 through RB-069 annotation-domain, workflow, export, artifact-integrity, provenance, prediction-import, correction, prediction-analysis, batch-import, project-operations routing, auth/RBAC/audit, batch-runner hardening, storage-cleanup, prediction QA metrics, editor decomposition, customer-trial handoff, trial-hardening, RB-076 local deployment dry-run, RB-077 iPad deferred-tracking, RB-085 through RB-098 crop-workflow slices, RB-104 legacy full-image editor removal, the RB-105 through RB-118 review-hardening sequence, RB-119/RB-120 documentation and decomposition governance, RB-121 through RB-129 role/workspace/editor refinements, the DESIGN/PERF editor and shell slices, EX-001 through EX-005 SaPen-CNN snapshot/materializer work, and the RB-130/RB-131 production-readiness remediation completed before RB-132.
...
- `scripts/materialize-sapen-cnn-dataset.mjs` - EX-005 local SaPen-CNN dataset materializer for audited manifest/private-ref handoff.
- `src/mask` - current label constants, mask buffers, serialization, patching, and overlay rendering.
- `src/server/domain/sapenCnnTrainingSnapshot.ts` - EX-003/EX-004 SaPen-CNN snapshot manifest builder.
...
- The crop workflow slice navigator uses `/app/projects/[projectId]/images/[imageId]/crop/slices/[sliceInstanceId]`, `src/features/editor/ImageCropSliceNavigatorClient.tsx`, `src/features/editor/CropEditorSliceNavigatorRailClient.tsx`, and `src/server/domain/cropSliceNavigator.ts` to show the source image with BBox overlays and per-slice crop/support/semantic/classification/readiness badges. RB-123 redirects selected current crops to `/crop/slices/[sliceInstanceId]/crops/[cropId]`, the unified crop annotation editor with annotation-family selection, status, review controls, and embedded whole-image navigation. RB-123/DESIGN-015 prevent accidental Sapwood/Heartwood vs Cu mixing through byte-derived family detection, save-time conflict guards, Support-as-Cu-label editing, and readiness blockers for conflicts. RB-101 embeds the navigator as the right rail of the crop editor.
...
- Project operations use route-addressable pages: `/app/projects/[projectId]` for the canonical image workspace plus status/actions, `/tasks` for correction queues, `/exports` for training and prediction-analysis exports, and `/prediction-imports` for prediction batch imports. The former `/app/projects/[projectId]/images` workspace redirects to the project root for compatibility.
...
- SaPen-CNN training snapshots use the logical `sapen_cnn_training` export target with `packageMode = "manifest_only"` and `manifestFormatVersion = sapen-annotate-cnn-training-dataset-v1`. The public manifest contains object refs only; private storage keys remain behind the audited owner/operator materialization refs endpoint and the local materializer writes `../sapen-cnn`-compatible crop classification, crop semantic, and full-image instance datasets.
...
- RB-086 adds rough BBox proposals for candidate slices, RB-087 adds derived crop generation, RB-088 adds crop support-mask editing, RB-089 adds crop semantic annotation, RB-090 adds auto classification suggestions with manual override provenance, RB-091 adds crop training export, RB-092 adds shared crop readiness/review integration, and RB-094 adds persisted BBox set confirmation. General interactive crop-mask reprojection into source-image space remains deferred; the SaPen-CNN snapshot builder performs a bounded full-image instance-mask reconstruction from approved crop support geometry for the EX-003/EX-004 export contract.
