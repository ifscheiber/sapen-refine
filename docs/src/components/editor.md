...
- BBox proposal editing also uses Pointer Events and stores integer source-image pixel rectangles. In BBox-stage mode the toolbar exposes icon-only Add BBox, Delete selected BBox, Zoom, and Fit controls; resize is direct manipulation through selected BBox handles. Zoom is `1%` to `100%` relative to the BBox working preview; `100%` means one preview pixel per CSS pixel, slider zoom preserves the visible center where possible, and Fit centers the fitted image where possible. Selection/move/resize happen on the working preview canvas and are mapped back to original source-image coordinates before save; protected BBoxes can still be selected but cannot be deleted or geometry-edited. BBox counts, save state, workflow state, current/missing/stale slice counts, `Unlock BBox editing`, and optional `Regenerate slices` live in the right rail. Leaving the BBox tab through Semantic Masks or Export Readiness automatically confirms the current valid BBox set and ensures current crops for missing/stale slices. The BBox stage uses the same `Annotation Editor` shell and local tab row as crop semantic/support editing.
