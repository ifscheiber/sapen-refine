...
- `src/server/domain/sliceBboxes.ts` - RB-086/DESIGN-006 BBox proposal list/create/replace/delete helpers, source-image geometry validation, overlap/dependency guardrails, append-only versioning, and audit events.
...
- `runStorageCleanup` implements the RB-066 admin-only dry-run/execute cleanup path for temporary batch staging and abandoned presigned upload objects, with RB-114/RB-138 consistency and optional project-scoped deep checksum reporting.
...
- `processDueExportJobsForUser` processes due RB-112 export jobs for projects where the job account has the RB-140 `export:processJobs` capability; export creation and download remain target-specific.
- `listSliceBoundingBoxesForUser`, `createSliceBoundingBoxForUser`, `replaceSliceBoundingBoxForUser`, `deleteSliceBoundingBoxForUser`, and `confirmImageBBoxSetForUser` implement the RB-086/RB-094/DESIGN-006 source-image BBox proposal and image-level confirmation service layer. List responses remain backward-compatible and add `bboxIssues`, `bboxSummary`, and per-box dependency/protection metadata for the editor toolbar. Protected BBox replace/delete calls can explicitly invalidate active downstream slice annotations by superseding artifact/classification versions instead of deleting history.
...
- Slice BBox services create crop-planning proposal versions only. They validate source-image pixel bounds, reject overlapping active BBox sets, append replacement/deletion versions, and do not create support masks or export-ready ground truth. BBoxes with semantic mask, support/instance mask, or classification versions are protected against delete and geometry replacement; derived crops alone remain append-only lineage and do not lock geometry.
