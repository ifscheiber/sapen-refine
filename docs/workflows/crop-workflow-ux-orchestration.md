...
The BBox stage is the first workflow stage. It shows the source image, provides an icon-only BBox toolbar for source-image BBox proposal drawing, safe deletion, zoom, and fit. Selection, move, and resize are direct canvas interactions; the separate Resize mode is removed. For large images, the stage may render a downscaled working preview to reduce browser load; the saved BBoxes remain original source-image coordinates, and crop generation still uses the original image. BBox counts, save state, protection, workflow state, and current/missing/stale slice counts are shown in the right rail instead of full-width workflow strips above the canvas.
...
RB-103 makes this edit/re-prepare loop reachable from crop annotation editors. Opening `/crop/bboxes` after confirmation shows the confirmed BBoxes but keeps mutation controls locked until the user clicks `Unlock BBox editing` in the right rail. Navigation-only re-entry keeps the workflow in `BBOX_CONFIRMED`; replacing or deleting BBoxes after the explicit unlock marks the set `BBOX_NEEDS_UPDATE`, and the next BBox-tab exit or right-rail `Regenerate slices` action reconfirms and ensures current crops.
