...
The BBox stage is the first workflow stage. It shows the source image, provides an icon-only BBox toolbar for safe deletion, zoom, and fit, and uses direct Select/Edit/Draw canvas interaction. Dragging empty image space creates a new BBox; selecting, moving, and resizing existing BBoxes are direct canvas interactions. For large images, the stage may render a downscaled working preview to reduce browser load; the saved BBoxes remain original source-image coordinates, and crop generation still uses the original image. BBox counts, save state, selected-BBox lock state, protection, workflow state, and current/missing/stale slice counts are shown in the right rail instead of full-width workflow strips above the canvas.

Preparing slices records the existing image-level BBox confirmation workflow intent. The browser now performs that preparation automatically when users leave the BBox tab for Semantic Masks or Export Readiness, then ensures current derived crops for missing or stale valid BBoxes. This does not approve BBoxes as review artifacts and does not make a slice training-ready.
...
RB-103 makes this edit/re-prepare loop reachable from crop annotation editors. Opening `/crop/bboxes` after confirmation shows the confirmed BBoxes and keeps editable BBoxes directly mutable. Navigation-only re-entry keeps the workflow in `BBOX_CONFIRMED`; replacing or deleting BBoxes marks the set `BBOX_NEEDS_UPDATE`, and the next BBox-tab exit reconfirms and ensures current crops.

DESIGN-006 adds BBox-stage safety guardrails. Active BBoxes must not overlap; overlap issues are shown in the right-rail status and block slice preparation/opening. BBoxes with active semantic mask, support/instance mask, or classification versions are individually locked against deletion and geometry mutation until the user confirms invalidation. Confirmation marks downstream versions `SUPERSEDED` and clears active support pointers; it does not delete annotation history or object-storage artifacts. Existing derived crops alone do not block BBox replacement because crop history is append-only and stale/current crop state is already represented separately.
