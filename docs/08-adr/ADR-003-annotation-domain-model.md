...
- Active-learning and model preprediction fit through task priority, task reason, confidence/uncertainty, and prediction provenance; RB-056 through RB-061 implement the first proposal/correction path.

## Deferred Work

- Full API/UI workflow depth beyond the current MVP slices.
- Advanced export filters/history/job handling.
- Always-on prediction workers, staging cleanup, metrics dashboards, and slice-classification prediction correction.
