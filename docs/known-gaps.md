...
- RB-085 documents the crop-based slice annotation workflow. RB-086 implements the first runtime slice: source-image BBox proposals are versioned, linked to slice instances, and validated against image dimensions. Derived crop generation, crop support-mask editing, crop-constrained semantics, auto classification, crop-aware exports, and crop review integration remain deferred to RB-087 through RB-092.
...
- Editor UX is consolidated under `src/features/editor`; RB-068 splits the previous monolithic client into smaller modules, RB-070 adds explicit eraser UX, RB-080/RB-081 harden full-resolution save readiness/payload behavior, RB-085 defines the crop-based scalable path, and RB-086 adds BBox proposal mode. Advanced iPad zoom/pan gestures, tiled/downscaled working masks for memory-constrained devices, edit-session/multi-tab warnings, crop support editing, and deeper drawing/state hook decomposition remain deferred.
...
- RB-085/RB-086 start the crop-based slice annotation sprint with the design contract and persistent BBox slice proposals. RB-087 through RB-092 remain the next crop-runtime slices.
