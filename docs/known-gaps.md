...
This page summarizes known limitations after the RB-040 through RB-052 baseline, metadata workflow, slice-support workflow, and review/approval workflow work.
...
- Admin export generation and manifest reproducibility are not implemented, though `ExportBatch`/`ExportItem` persistence exists for RB-053 and RB-052 now exposes approved-version readiness.
