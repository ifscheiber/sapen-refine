...
- RB-085 documents the planned crop-based slice annotation workflow: BBox proposals seed derived crops, pixel-perfect support masks remain mandatory training geometry, semantic masks are constrained by support, Copper semantic masks are not support geometry, and crop-aware exports must preserve source-image coordinate provenance. Runtime implementation is deferred to RB-086 through RB-092.
