...
RB-056 adds `ModelRun`, `PredictionRun`, and `PredictionArtifactProvenance`, but those records are provenance only. They do not change export eligibility, do not mark `PREDICTION_MASK` artifact versions as ground truth, and do not create slice-classification labels. Prediction-analysis exports remain a separate future target.
