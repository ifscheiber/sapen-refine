...
- `SliceInstance` and `SliceClassificationVersion` provide the persistence baseline for RB-051. RB-086 adds `SliceBoundingBoxVersion` for append-only source-image BBox proposal history linked to slice instances. RB-094 adds `ImageCropWorkflowState` for image-level BBox set confirmation snapshots. RB-087 adds `DerivedSliceCrop` for append-only private crop PNG versions linked to source images, BBox versions, and slice instances. RB-088 links crop support artifact versions to those crop and slice rows. RB-089/RB-100 links crop semantic artifact versions to the crop, slice, optional support mask version, and semantic mode. RB-090 adds manual/auto classification source, derivation reason, metadata, and optional links from classifications back to the semantic mask, support mask, and crop that produced an auto suggestion.
...
- `ExportBatch` and `ExportItem` persist RB-053 full-image training export batches, RB-091 crop training export batches, and RB-060/RB-067 prediction-analysis export batches, RB-112 job state/lease/retry metadata, manifest/package metadata, QA metric summary metadata, warnings, actor attribution, exact exported version/provenance references, and crop-package `derivedCropId` references.
...
- image-level BBox set confirmation writes `ImageCropWorkflowState` rows through `/api/images/[imageId]/slice-bboxes/confirm`; later BBox edits mark confirmed sets as `NEEDS_UPDATE`,
...
- RB-086 implements persistent source-image BBox proposal versions. RB-087 implements derived crop generation with configurable 32 px default padding, clipped source rectangles, private PNG storage, and app-mediated reads. RB-088 implements crop support-mask editing and artifact lineage. RB-089/RB-100 implements mode-aware crop semantic masks with optional support-version lineage. RB-090 implements draft auto slice classification suggestions from crop semantic masks plus manual override provenance. RB-091 implements crop-aware training export packages, RB-092 centralizes crop readiness/review integration without schema changes, and RB-094 implements persisted image-level BBox set confirmation state.
