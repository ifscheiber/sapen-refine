...
- RB-086 implements persistent source-image BBox proposal versions. RB-087 implements derived crop generation with configurable 32 px default padding, clipped source rectangles, private PNG storage, and app-mediated reads. RB-088 implements crop support-mask editing and artifact lineage. RB-089 implements support-constrained crop semantic masks with exact support-version lineage. RB-090 implements draft auto slice classification suggestions from crop semantic masks plus manual override provenance. RB-091 implements crop-aware training export packages, RB-092 centralizes crop readiness/review integration without schema changes, and RB-094 implements persisted image-level BBox set confirmation state.
