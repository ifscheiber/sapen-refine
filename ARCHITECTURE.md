...
- Crop semantic masks and classification suggestions: `/app/projects/[projectId]/images/[imageId]/slices/[sliceInstanceId]/crops/[cropId]/semantic` edits crop-sized semantic masks through `/api/slice-crops/[cropId]/semantic-mask`; saved versions use `CROP_PIXEL`, apply mode-aware support policy, derive Sap/Heartwood support geometry from semantic foreground, require explicit support for Copper readiness/export, and append draft `AUTO_FROM_SEMANTIC_MASK` slice-classification suggestions from the saved semantic bytes.
