...
- BBox slice proposals: the editor BBox mode stores source-image pixel rectangles through `/api/images/[imageId]/slice-bboxes` and `/api/slice-bboxes/[bboxVersionId]`. BBoxes are append-only proposal versions linked to `SliceInstance`; they are not support masks or export-ready ground truth. `/app/projects/[projectId]/images/[imageId]/crop/bboxes` is the staged BBox workflow route, and `/api/images/[imageId]/slice-bboxes/confirm` records image-level BBox set confirmation state without treating BBoxes as ground-truth approval.
