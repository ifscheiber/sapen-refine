...
- BBox slice proposals and navigation: the editor BBox mode stores source-image pixel rectangles through `/api/images/[imageId]/slice-bboxes` and `/api/slice-bboxes/[bboxVersionId]`. BBoxes are append-only proposal versions linked to `SliceInstance`; they are not support masks or export-ready ground truth. `/app/projects/[projectId]/images/[imageId]/crop/bboxes` is the staged BBox workflow route, `/api/images/[imageId]/slice-bboxes/confirm` records image-level BBox set confirmation state without treating BBoxes as ground-truth approval, and `/app/projects/[projectId]/images/[imageId]/crop/slices/[sliceInstanceId]` shows the whole-image slice navigator with per-slice crop/readiness status.
...
- Crop semantic masks and classification suggestions: `/app/projects/[projectId]/images/[imageId]/slices/[sliceInstanceId]/crops/[cropId]/semantic` edits crop-sized semantic masks through `/api/slice-crops/[cropId]/semantic-mask`; saved versions use `CROP_PIXEL`, require the current crop support mask, link to the exact support mask version, reject semantic foreground outside support, and append draft `AUTO_FROM_SEMANTIC_MASK` slice-classification suggestions from the saved semantic bytes.
