...
Known workflow gaps include advanced export filters/history/job handling, reviewer dashboards/bulk review, stronger upload checksum/dimension validation, multi-slice support, and runtime prediction import/active-learning workflows. `MaskKind.PREDICTION` and `MaskKind.REFINED` are removed from the active schema; "refine" is reserved for a future prediction-correction mode, not the product name.
