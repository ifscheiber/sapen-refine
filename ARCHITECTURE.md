...
- Annotation entry: image lists and metadata pages route annotation work to `/app/projects/[projectId]/images/[imageId]/crop`. RB-104 removed the legacy `/app/projects/[projectId]/images/[imageId]/edit` product route; historical references to that route are not current workflow entry points.
- BBox slice proposals and navigation: `/app/projects/[projectId]/images/[imageId]/crop` redirects to the BBox stage or slice navigator based on persisted workflow state. `/app/projects/[projectId]/images/[imageId]/crop/bboxes` stores source-image pixel rectangles through `/api/images/[imageId]/slice-bboxes` and `/api/slice-bboxes/[bboxVersionId]`. BBoxes are append-only proposal versions linked to `SliceInstance`; they are not support masks or export-ready ground truth. `/api/images/[imageId]/slice-bboxes/confirm` records image-level BBox set confirmation state without treating BBoxes as ground-truth approval, `/app/projects/[projectId]/images/[imageId]/crop/slices` shows the whole-image slice navigator, and `/app/projects/[projectId]/images/[imageId]/crop/slices/[sliceInstanceId]/crops/[cropId]` shows the selected crop workbench with per-slice crop/readiness status.
...
- Crop support masks: `/app/projects/[projectId]/images/[imageId]/crop/slices/[sliceInstanceId]/crops/[cropId]/support` edits crop-sized binary support masks through `/api/slice-crops/[cropId]/support-mask`; saved versions use `CROP_PIXEL` and link to the source image, slice instance, and derived crop.
...
- Safe append-only version allocation for concurrent artifact, BBox, crop, and classification writers.
